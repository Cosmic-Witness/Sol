...
def canonical_validation_files() -> tuple[list[Path], Path]:
    annotations = next(
        Path("/kaggle/input").rglob("MAGFiLO_1.0_Annotations_kaggle2026_train.json")
    )
    coco = json.loads(annotations.read_text(encoding="utf-8"))
    by_site: dict[str, list[str]] = {}
    for stem in sorted({str(row["file_name"]) for row in coco["images"]}):
        by_site.setdefault(stem.rsplit(".", 1)[0][-2:], []).append(stem)
    rng = np.random.default_rng(2026)
    selected: list[str] = []
    for site in sorted(by_site):
        stems = by_site[site]
        rng.shuffle(stems)
        selected.extend(stems[:max(1, round(len(stems) * 0.15))])
    image_dir = next(Path("/kaggle/input").rglob("train_images"))
    files = [image_dir / name for name in sorted(selected)]
    if len(files) != 106 or any(not path.is_file() for path in files):
        fail(f"canonical validation discovery failed: {len(files)} files")
        ...
        result[name] = [
            {
                "size": [2048, 2048],
                "counts": (
                    row["counts"].decode("ascii")
                    if isinstance(row["counts"], bytes)
                    else str(row["counts"])
                ),
            }
            for row in rows
...
def main_validation() -> None:
    ...
    detector.eval()
    refiner.eval()
    files, annotations = canonical_validation_files()
    variants: dict[str, dict[str, list[dict[str, Any]]]] = {
        "anchor": {}, "attached": {}, "candidate": {}
    }
    stats: list[dict[str, Any]] = []
    for index, image_path in enumerate(files, 1):
        image = cv2.imread(str(image_path), cv2.IMREAD_GRAYSCALE)
        if image is None or image.shape != (2048, 2048):
            fail(f"validation image decode mismatch: {image_path.name}")
        anchor, attached, candidate, row = infer_observation(
            detector, refiner, image, device=device
        )
        name = image_path.name
        variants["anchor"][name] = anchor
        variants["attached"][name] = attached
        variants["candidate"][name] = candidate
        stats.append({"photograph": name, **row})
        if index % 10 == 0 or index == len(files):
            print(json.dumps({"event": "heartbeat", "completed": index,
                              "total": len(files), "elapsed_seconds":
                              time.perf_counter() - started}), flush=True)
    payload = {
        "split_seed": 2026,
        "annotation_md5": md5_file(annotations),
        "weights_md5": EXPECTED_MD5,
        "training_executed": False,
        "variants": {
            name: serialise_predictions(predictions)
    ...
    report = {
        "images": len(files),
        "rows": {name: sum(map(len, predictions.values()))
                 for name, predictions in variants.items()},
        "elapsed_seconds": time.perf_counter() - started,
        "stats": stats,
        "training_executed": False,
    ...
    print(json.dumps({key: value for key, value in report.items()
                      if key != "stats"}, sort_keys=True), flush=True)
