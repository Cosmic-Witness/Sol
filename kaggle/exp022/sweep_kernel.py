"""CPU kernel: widen exp_010's operating-point grid past the edge it stopped at."""
...
def main() -> None:
    ...
    if not caches:
        raise SystemExit("exp_015 candidate cache not attached")
    print(f"cache {caches[0]}", flush=True)
    ...
    run([sys.executable, "-m", "experiments.exp_022_opoint.src.sweep",
         "--candidates", caches[0],
         "--annotations", root / "train" / ANNOTATION_NAME,
         "--out", OUT_DIR / "opoint.json"])
